...
bool ImportUiController::importFromSubscription(const QString &url)
{
    ErrorCode errorCode = m_importController->importSubscription(url);
    if (errorCode != ErrorCode::NoError) {
        emit importErrorOccurred(errorCode, false);
        return false;
    }
    // importFinished is forwarded from the core controller via the existing connection.
    return true;
}
