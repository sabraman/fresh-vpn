...
PageType {
    ...
    property bool subscriptionInProgress: false
    ...
    Connections {
        ...
        function onImportFinished() {
            if (!root.subscriptionInProgress) {
                return
            }
            root.subscriptionInProgress = false
        ...
        function onImportErrorOccurred(error, goToPageHome) {
            ...
            root.subscriptionInProgress = false
            PageController.showBusyIndicator(false)
            // The error message itself is shown by the global handler in PageStart.qml.
        }
    }
    ...
    ListViewType {
        ...
        header: ColumnLayout {
            ...
            ParagraphTextType {
                ...
                text: qsTr("Insert the key or subscription link, add a configuration file or scan the QR-code")
            }
            ...
            BasicButtonType {
                ...
                clickedFunc: function() {
                    var text = textKey.textField.text.trim()
                    if (text.startsWith("https://") || text.startsWith("http://")) {
                        root.subscriptionInProgress = true
                        PageController.showBusyIndicator(true)
                        var started = ImportController.importFromSubscription(text)
                        if (!started) {
                            root.subscriptionInProgress = false
                            PageController.showBusyIndicator(false)
                        }
