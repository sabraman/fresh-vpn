...
namespace {
    // Fresh fork: third-party gateway phone-home (gw.amnezia.org) neutralized.
    // Self-hosted use imports configs directly and never calls the Amnezia gateway;
    // empty default => getGatewayEndpoint() returns "" => no outbound call by default.
