                                                         ...
                                                         CaptchaInfo *captchaInfoOut, ProtocolData *usedProtocolDataOut)
{
    ...
    // Upstream: propagate captcha info so the UI can solve it, and refresh on api-key expiry.
    ErrorCode errorCode = updateServiceFromGateway(serverId, "", true, captchaInfoOut, usedProtocolDataOut);
    if (errorCode == ErrorCode::NoError) {
        return ErrorCode::NoError;
    }
    ...
    if (errorCode == ErrorCode::ApiCaptchaRequiredError
        || errorCode == ErrorCode::ApiCaptchaInvalidError
        || errorCode == ErrorCode::ApiCaptchaRefreshError) {
        return errorCode;
    }
