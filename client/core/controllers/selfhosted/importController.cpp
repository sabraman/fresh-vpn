...
namespace
{
    // Hard cap on the subscription response body to defend against a malicious or
    // misbehaving server streaming an unbounded payload (OOM defense).
    constexpr qint64 kMaxSubscriptionBytes = 2 * 1024 * 1024; // 2 MB
}

ErrorCode ImportController::importSubscription(const QString &subscriptionUrl)
{
    const QUrl url = QUrl::fromUserInput(subscriptionUrl.trimmed());
    if (!url.isValid() || (url.scheme() != "http" && url.scheme() != "https")) {
        return ErrorCode::ImportInvalidConfigError;
    }

    // Only one subscription import in flight at a time.
    if (m_subscriptionReply != nullptr) {
        return ErrorCode::ImportInvalidConfigError;
    }
    ...
    if (m_subscriptionTimer == nullptr) {
        m_subscriptionTimer = new QTimer(this);
        m_subscriptionTimer->setSingleShot(true);
        QObject::connect(m_subscriptionTimer, &QTimer::timeout, this, [this]() {
            m_subscriptionTimedOut = true;
            if (m_subscriptionReply != nullptr) {
    ...
    m_subscriptionTimedOut = false;
    m_subscriptionSizeExceeded = false;

    QNetworkRequest request(url);
    request.setHeader(QNetworkRequest::UserAgentHeader, QByteArrayLiteral("FreshVPN"));
    request.setAttribute(QNetworkRequest::RedirectPolicyAttribute, QNetworkRequest::NoLessSafeRedirectPolicy);

    m_subscriptionReply = m_networkManager->get(request);

    // Abort early if the server streams more than the cap (OOM defense).
    QObject::connect(m_subscriptionReply, &QNetworkReply::downloadProgress, this,
                     [this](qint64 bytesReceived, qint64 bytesTotal) {
                         if (bytesReceived > kMaxSubscriptionBytes || bytesTotal > kMaxSubscriptionBytes) {
                             m_subscriptionSizeExceeded = true;
                             if (m_subscriptionReply != nullptr) {
                                 m_subscriptionReply->abort();
                             }
                         }
    ...
    QObject::connect(m_subscriptionReply, &QNetworkReply::finished, this,
                     &ImportController::onSubscriptionReplyFinished);

    m_subscriptionTimer->start(15000);

    // The network request runs asynchronously on the Qt event loop; the result is
    // delivered via importFinished / importErrorOccurred. The UI thread is never
    // blocked here (this fixes the previous synchronous QEventLoop freeze / Android ANR).
    return ErrorCode::NoError;
}

void ImportController::onSubscriptionReplyFinished()
{
    if (m_subscriptionTimer != nullptr) {
        m_subscriptionTimer->stop();
    }

    QNetworkReply *reply = m_subscriptionReply;
    ...
    if (m_subscriptionSizeExceeded) {
        m_subscriptionSizeExceeded = false;
        emit importErrorOccurred(ErrorCode::ImportInvalidConfigError, false);
        return;
    }

    if (m_subscriptionTimedOut) {
        m_subscriptionTimedOut = false;
        emit importErrorOccurred(ErrorCode::ApiConfigTimeoutError, false);
        return;
    }

    if (reply->error() != QNetworkReply::NoError) {
        const bool sslError = (reply->error() == QNetworkReply::SslHandshakeFailedError);
        emit importErrorOccurred(sslError ? ErrorCode::ApiConfigSslError : ErrorCode::ApiConfigDownloadError, false);
        return;
    }

    // Redirect safety: after following any redirects, the final URL must still be http(s).
    ...
    if (body.isEmpty()) {
        emit importErrorOccurred(ErrorCode::ApiConfigEmptyError, false);
        return;
    }

    const ErrorCode code = addServersFromSubscriptionBody(body);
    if (code == ErrorCode::NoError) {
        emit importFinished();
    } else {
        emit importErrorOccurred(code, false);
    }
}

ErrorCode ImportController::addServersFromSubscriptionBody(const QByteArray &body)
{
    // Remnawave-style subscription: body is base64 of newline-separated links
    // (vless://, ss://, trojan://, ...). Some panels return raw links instead.
    ...
    if (text.contains("://")) {
        ...
        if (!QString::fromUtf8(decoded).contains("://")) {
            decoded = QByteArray::fromBase64(text.toUtf8(),
                                             QByteArray::Base64UrlEncoding | QByteArray::OmitTrailingEquals);
        }
        links = QString::fromUtf8(decoded);
    }
    ...
    // Signature used to skip duplicates: host + default container.
    auto signatureOf = [](const QJsonObject &config) -> QString {
        const QString host = config.value(configKey::hostName).toString();
        const QString container = config.value(configKey::defaultContainer).toString();
        return host + "|" + container;
    };

    // Build the set of servers already present so a repeated import does not duplicate them.
    ...
    for (const QString &id : existingIds) {
        const QJsonObject existing = m_serversRepository->serverJson(id);
        if (!existing.isEmpty()) {
            existingSignatures.insert(signatureOf(existing));
        }
    }

    const QStringList lines = links.split(QRegularExpression("[\\r\\n]+"), Qt::SkipEmptyParts);
    QSet<QString> batchSignatures;
    int imported = 0;
    ...
    for (const QString &rawLine : lines) {
        ...
        ImportResult result = extractConfigFromData(line);
        if (result.errorCode != ErrorCode::NoError || result.config.isEmpty()) {
            continue;
        }

        // Security: never silently bulk-add a config that triggered the malicious-string
        // check (e.g. an OpenVPN config carrying script hooks). Those must be imported
        // one-by-one through the normal path where the user sees the warning first.
        if (!result.maliciousWarningText.isEmpty()) {
            skippedUnsafe++;
            qWarning() << "importSubscription: skipped a config requiring manual review (possible embedded scripts)";
            continue;
        }

        // Dedup by host + container: re-importing the same node must not create duplicates.
        const QString host = result.config.value(configKey::hostName).toString();
        const QString sig = signatureOf(result.config);
        if (!host.isEmpty() && (existingSignatures.contains(sig) || batchSignatures.contains(sig))) {
            skippedDuplicate++;
            continue;
        }
        batchSignatures.insert(sig);

        m_serversRepository->addServer(QString(), result.config,
                                       serverConfigUtils::configTypeFromJson(result.config));
        imported++;
    }
    ...
    // Nothing new added. If everything was a duplicate (and nothing unsafe), report that.
    if (skippedDuplicate > 0 && skippedUnsafe == 0) {
